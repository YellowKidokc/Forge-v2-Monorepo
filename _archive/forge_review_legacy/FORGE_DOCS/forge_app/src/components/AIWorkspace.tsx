...
import { getAiProvider, hasApiKey, missingKeyMessage, providerLabel, runAiChat, setAiProvider, ChatTurn } from '../lib/ai';
...
const ChatPane = ({
  ...
  const sendMessage = async () => {
    ...
    const systemPrompt = `${defaultPrompt}${contextBlock(contextMode, workspaceContext)}`;
...
const AIWorkspace = ({ activeFile, workspaceContext, onOpenSettings }: AIWorkspaceProps) => {
  ...
  const workspaceChat = (
    <ChatPane
      ...
      provider={provider}
      badge="Context-aware"
      defaultPrompt="You are FORGE Workspace AI. Focus on execution, structure, and next best actions."
    />
  );
  ...
  const privateChat = (
    <ChatPane
      ...
      provider={provider}
      badge="Isolated"
      defaultPrompt="You are FORGE Private AI. Assist with private ideation unless the user asks to link to workspace."
    />
  );
      ...
      <div className="flex-1 min-h-0 flex flex-col p-2 gap-2">
        <div className="border border-forge-steel rounded bg-[#151515] px-3 py-2 flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            {(['anthropic', 'openai'] as AiProvider[]).map((item) => (
              <button
                key={item}
                onClick={() => {
                  setAiProvider(item);
